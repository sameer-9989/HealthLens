...
const VirtualNursingAssistantOutputSchema = z.object({
  ...
  suggestedAction: z.string().optional().describe('A suggested action for the user, e.g., a mindfulness exercise step.')
});
...
const prompt = ai.definePrompt({
  ...
  6.  **Stress Relief Yoga Suggestion:**
      *   If the user mentions stress and asks for yoga, or directly requests stress relief yoga:
          *   Suggest a type of yoga routine (e.g., "Gentle stretching yoga can be very helpful for stress." or "You could try some simple desk yoga poses.").
          *   You can describe 1-2 very simple poses textually if asked (e.g., "A simple neck stretch: Gently tilt your head to one side, hold for 15-20 seconds, then switch. Or a shoulder roll: Inhale and lift your shoulders to your ears, then exhale and roll them back and down. Repeat a few times.").
          *   Do NOT pretend to show videos. You can say "There are many great videos online for [yoga type] if you'd like to follow a visual guide."

  7.  **Safety and Disclaimers:**
      *   ALWAYS include a disclaimer if providing any health-related information or suggestions: "Remember, I'm an AI assistant and this isn't medical advice. Please consult with your doctor or a healthcare professional for any health concerns or before making changes to your treatment."
      *   If the user's message indicates a serious medical emergency, respond: "If you are experiencing a medical emergency, please call emergency services or go to the nearest emergency room immediately." and provide no further health advice.

  Generate the 'response' field as your main conversational reply. Use 'interactionWarning' and 'suggestedAction' for specific scenarios as described.
...
const virtualNursingAssistantFlow = ai.defineFlow(
  ...
  async input => {
    ...
    const emergencyPhrases = ["emergency", "urgent help", "can't breathe", "chest pain", "suicidal"];
    ...
    // Ensure a general disclaimer is part of the response if not explicitly handled for interactions
    if (!output.interactionWarning && output.response && !output.response.toLowerCase().includes("consult your doctor") && !output.response.toLowerCase().includes("medical advice")) {
        const disclaimer = " Remember, this is not medical advice. Please consult your doctor for any health concerns.";
